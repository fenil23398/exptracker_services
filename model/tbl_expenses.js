...
var expenses = {
    ...
    getExpenseByColor: function(exp, callback) {
        return db.query("select e.*,s.sub_cat_name,s.icon_image  from exp_tbl e,sub_category_tbl s where e.fk_scat_id = s.sub_cat_id and e.fk_user_email=? and e.colour_name=?", [exp.fk_user_email, exp.colour_name], callback)
    },
    ...
    updateExpenses: function(id, exp, callback) {
        return db.query("update exp_tbl set fk_user_email=?,fk_scat_id=?,expense_date=?,expense_amt=?,colour_name=?,exp_note=? where expense_id=?", [exp.fk_user_email, exp.fk_scat_id, exp.expense_date, exp.expense_amt, exp.colour_name, exp.exp_note, id], callback);
    },
