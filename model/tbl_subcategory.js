...
var scategories = {
    ...
    getAllSubCatByJoin: function(id, demo, callback) {
        console.log("hello")
        return db.query("select s.*,e.*,c.* from sub_category_tbl s,exp_tbl e,category_tbl c where s.fk_cat_id=c.cat_id and e.fk_scat_id=s.sub_cat_id and e.fk_user_email=? and s.fk_cat_id=?", [id, cat.fk_cat_id], callback)
    }
